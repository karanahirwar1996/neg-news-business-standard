...
def positive_news():
    ...
    final = filtered_df.loc[(filtered_df['Deep Score']<0) & (filtered_df['Normal Score'] <= 0)].reset_index(drop=True)
    ...
    msg["Subject"] = f"✨ Daily Nagetive News Digest-Business Standard- {current_date} ({positive_news_count} uplifting articles out of {len(filtered_df)})✨"
